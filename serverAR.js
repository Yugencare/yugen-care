...
app.get("/*", (req, res, next) => {
  fs.readFile(indexPath, "utf8", async (err, htmlData) => {
    ...
    if (post) {
      ...
      var SeoFields = "";
      if (post.meta) {
        SeoFields = `<title>${post.meta.title_ar}</title><meta name="description" content="${post.meta.description_ar}"/>`;
      }
      if (post.properties_ar) {
        const properties = JSON.parse(post.properties_ar);
      ...
      if (post.schema_ar && post.schema_ar.length !== 0) {
        var allschemas = "";

        for (let i = 0; i < post.schema_ar.length; i++) {
          allschemas =
            allschemas +
